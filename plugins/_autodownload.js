        ...
        if (global.db.data.users[m.sender].limit > 0) {
            ...
            if (data.result.video.length > 1) {
                ...
                for (let v of data.result.video) {
                    await conn.sendFile(m.chat, v, null, `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
                    await _sleep(3000);
                }
            } else {
                await conn.sendMessage(
                    m.chat,
                    {
                        ...
                        caption: `Proses Berhasil* : ${(new Date() - old) * 1} ms`,
                    },
                    {
                        mention: m,
                    }
            ...
            if (!data.result.video || data.result.video.length === 0) {
                ...
                if (data.result.images && data.result.images.length > 0) {
                    ...
                    for (let img of data.result.images) {
                        await conn.sendFile(m.chat, img, null, `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
                        await _sleep(3000);
                    }
                    return;
                }
    ...
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            ...
            for (let i = 0; i < Math.min(limitnya, res.result.length); i++) {
                await _sleep(3000);
                conn.sendFile(m.chat, res.result[i].url, null, `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
            }
            global.db.data.users[m.sender].limit -= 1;
        } else {
            conn.reply(m.chat, "Limit kamu habis!", m);
    ...
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            ...
            if (Array.isArray(urls)) {
                ...
                if (videoUrl) {
                    global.db.data.users[m.sender].limit -= 1;
                    conn.sendFile(m.chat, videoUrl, "fb.mp4", `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
                } else {
        ...
        if (global.db.data.users[m.sender].limit > 0) {
            ...
            if (res.result && res.result.mediaURLs) {
                ...
                for (const url of mediaURLs) {
                    ...
                    await _sleep(3000);
                    conn.sendFile(m.chat, buffer, null, `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
                }
            } else {
...
async function _threads(url, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            ...
            if (video) {
                try {
                    await conn.sendFile(m.chat, video.download_url, "threads.mp4", `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
                } catch (e) {
                    throw "Media video tidak ditemukan!";
                }
            } else if (foto) {
                try {
                    await conn.sendFile(m.chat, foto, "threads.jpeg", `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
                } catch (e) {
...
async function _capcut(url, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            ...
            global.db.data.users[m.sender].limit -= 1;
            await conn.sendFile(m.chat, video, "capcut.mp4", `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
        } else {
            conn.reply(m.chat, "Limit kamu habis!", m);
        }
    } catch (e) {
...
async function _snackvideo(url, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            ...
            await conn.sendFile(m.chat, media, null, `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
            global.db.data.users[m.sender].limit -= 1;
        } else {
            conn.reply(m.chat, "Limit kamu habis!", m);
        }
        ...
        if (global.db.data.users[m.sender].limit > 0) {
            ...
            if (res.result && res.result.data) {
                ...
                if (media_type === "video/mp4") {
                    ...
                    await conn.sendFile(m.chat, image, "pindl.jpeg", `Proses Berhasil* : ${(new Date() - old) * 1} ms`, m);
                }
            } else {
