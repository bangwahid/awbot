...
handler.before = async function (m, { conn }) {
    ...
    if (room.new) {
        ...
        room.killer = false
        room.kata = _kata
        room.chat = await this.reply(m.chat, `🎮 *GILIRAN BERMAIN*

👤 Saatnya: @${room.curr.split('@')[0]}

📌 Kata awal: *${_kata.toUpperCase()}*
🔎 Lanjutkan: *${room.filter(_kata).toUpperCase()}... ?*

💬 Jawab langsung di chat
❌ Ketik "nyerah" untuk menyerah`, 0, { contextInfo: { mentionedJid: member } })
    }
    ...
    if (room.curr == m.sender) {
        if (/nyerah/i.test(m.text)) {
            ...
            if (room.player.length == 1) {
                ...
                return this.reply(m.chat, `🏆 *BERHASIL*
...
Jawaban tidak boleh digunakan atau sudah digunakan atau tidak sesuai aturan!`)
        }
        ...
        room.chat = await this.reply(m.chat, `👍 *BENAR!*
...
💬 Jawab langsung di chat
❌ Ketik "nyerah" untuk menyerah`, m, { contextInfo: { mentionedJid: member } })
    } else {
        if (room.status === 'play') {
            return m.reply(room.eliminated.includes(m.sender) ? `⚠️ Kamu sudah tereliminasi! ` : `🚫 _Bukan giliranmu!_`)
        }
    }
    return true
}
